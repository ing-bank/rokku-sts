...
class AdminApiTest extends WordSpec with DiagrammedAssertions with ScalatestRouteTest with BeforeAndAfterAll {
        ...
        case "valid" =>
          Some(
            AuthenticationUserInfo(
              UserName("username"),
              Set(UserGroup("admins"), UserGroup("group2")),
              AuthenticationTokenId("tokenOk")
            )
          )
        case "notAdmin" =>
          Some(
  ...
  "Admin Api" should {
    "check response".that {
      "return OK if user is in admin groups and all FormFields are posted" in {
        Post(
          "/admin/npa",
          FormData("npaAccount" -> "testNPA", "awsAccessKey" -> "SomeAccessKey", "awsSecretKey" -> "SomeSecretKey")
        ) ~> validOAuth2TokenHeader ~> testRoute ~> check {
      ...
      "return Rejected if user is not in admin groups" in {
        Post(
          "/admin/npa",
          FormData("npaAccount" -> "testNPA", "awsAccessKey" -> "SomeAccessKey", "awsSecretKey" -> "SomeSecretKey")
        ) ~> notAdminOAuth2TokenHeader ~> testRoute ~> check {
